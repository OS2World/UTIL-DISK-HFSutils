 ...
 *
 * $Id: btree.c,v 1.18 1997/11/07 16:48:34 rob Exp $
 */
...
int bt_getnode(node *np)
{
  ...
  if (f_getblock(&bt->f, np->nnum, bp) == -1)
    goto fail;
...
int insertx(node *np, byte *record, int *reclen)
{
  ...
  switch (np->nd.ndType)
	...
	rec = HFS_NODEREC(*np, np->rnum);
      ...
      child.bt   = np->bt;
      child.nnum = d_getul(HFS_RECDATA(rec));

      if (bt_getnode(&child) == -1 ||
 ...
 */
int bt_insert(btree *bt, const byte *record, int reclen)
{
  node root;
...
int bt_search(btree *bt, const byte *key, node *np)
{
  ...
  while (1)
	...
	case ndIndxNode:
	  ...
	  rec = HFS_NODEREC(*np, np->rnum);
	  np->nnum = d_getul(HFS_RECDATA(rec));
	  break;
