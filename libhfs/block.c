 ...
 *
 * $Id: block.c,v 1.22 1997/11/07 16:48:46 rob Exp $
 */
...
{
  fprintf(stderr, "CACHE: vol 0x%lx \"%s\" hit/miss ratio = %.1f%%\n",
	  (unsigned long) cache->vol, cache->vol->mdb.drVN,
	  cache->hits * 100.0 / cache->misses);
}
...
int fillchain(hfsvol *vol, bucket **bptr, unsigned int *count)
{
  ...
  for (len = 0; len < HFS_BLOCKBUFSZ && bptr - start < *count; ++bptr)
    {
      if (INUSE(*bptr))
	continue;
  ...
  *count = bptr - start;
  ...
  if (len == 0)
  ...
  else
    {
      ...
      result = b_readpb(vol, vol->vstart + blist[0]->bnum, buffer, len);
      if (result == -1)
	goto fail;
      else if (result != len)
	ERROR(EIO, "failed to read all consecutive blocks");
  ...
  *count = bptr - start;
  ...
  if (len == 0)
	...
	memcpy(buffer[i], blist[i]->data, HFS_BLOCKSZ);
      ...
      result = b_writepb(vol, vol->vstart + blist[0]->bnum, buffer, len);
      if (result == -1)
	goto fail;
      else if (result != len)
	ERROR(EIO, "failed to write all consecutive blocks");
    }
...
# ifdef DEBUG
  if (INUSE(b))
    fprintf(stderr, "CACHE: reusing bucket containing "
...
/*
 * NAME:	block->readpb()
 * DESCRIPTION:	read block(s) from the physical medium (N.B. bypasses cache)
 */
...
# ifdef DEBUG
  fprintf(stderr, "READ: vol 0x%lx block %lu", (unsigned long) vol, bnum);
  if (blen > 1)
    fprintf(stderr, "+%u[..%lu]\n", blen - 1, bnum + blen - 1);
  else
    fprintf(stderr, "\n");
# endif
  ...
  if (os_seek(&vol->priv, bnum << HFS_BLOCKSZ_BITS, HFS_SEEK_SET) == -1)
    goto fail;

  nbytes = os_read(&vol->priv, bp, blen << HFS_BLOCKSZ_BITS);
  if (nbytes == -1)
    goto fail;

  if (nbytes == 0)
    ERROR(EIO, "read EOF on volume");
  else if (nbytes < HFS_BLOCKSZ)
    ERROR(EIO, "read incomplete block");

  return nbytes >> HFS_BLOCKSZ_BITS;
...
/*
 * NAME:	block->writepb()
 * DESCRIPTION:	write a block to the physical medium (N.B. bypasses cache)
 */
...
# ifdef DEBUG
  fprintf(stderr, "WRITE: vol 0x%lx block %lu", (unsigned long) vol, bnum);
  if (blen > 1)
    fprintf(stderr, "+%u[..%lu]\n", blen - 1, bnum + blen - 1);
  else
    fprintf(stderr, "\n");
# endif
  ...
  nbytes = os_write(&vol->priv, bp, blen << HFS_BLOCKSZ_BITS);
  if (nbytes == -1)
    goto fail;

  if (nbytes < HFS_BLOCKSZ)
...
unsigned long b_size(hfsvol *vol)
{
  ...
  nbytes = os_seek(&vol->priv, 0, HFS_SEEK_END);

  if (nbytes != -1 && (unsigned long) nbytes >= HFS_BLOCKSZ)
    return (unsigned long) nbytes >> HFS_BLOCKSZ_BITS;

  /* fall back on manual size detection */

  for (low = 0, high = 2879;
       high > 0 && b_readpb(vol, high, &b, 1) != -1;
       high <<= 1)
    low = high;
  ...
  if (high == 0)
    ERROR(EIO, "cannot determine size of medium");
