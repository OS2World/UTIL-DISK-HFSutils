 ...
 *
 * $Id: hfs.c,v 1.22 1997/11/08 03:42:04 rob Exp $
 */
...
hfsvol *hfs_mount(const char *path, int pnum, int flags)
{
  ...
  for (check = hfs_mounts; check; check = check->next)
    {
      if ((check->pnum == 0 || check->pnum == pnum) &&
	  v_same(check, path) == 1)
	{
	  ...
	  if (((check->flags & HFS_VOL_READONLY) &&
	      ...
	      (! (check->flags & HFS_VOL_READONLY) &&
	       (flags & (HFS_MODE_RDWR | HFS_MODE_ANY))))
	    {
	      vol = check;
	      goto done;
	    }
		    ...
		    const char *type, const char *creator)
{
  ...
  found = v_resolve(&vol, path, cat, &file->parid, file->name, 0);
  if (found == -1 || file->parid == 0)
    goto fail;
  ...
  /* create file `file->name' in parent `file->parid' */
  ...
  cat->cdrType   = cdrFilRec;
  cat->cdrResrv2 = 0;

  cat->u.fil.filFlags = 0;
  cat->u.fil.filTyp   = 0;

  memset(&cat->u.fil.filUsrWds, 0, sizeof(cat->u.fil.filUsrWds));

  cat->u.fil.filUsrWds.fdType    = d_getsl((const unsigned char *) type);
  cat->u.fil.filUsrWds.fdCreator = d_getsl((const unsigned char *) creator);

  cat->u.fil.filFlNum  = id;
  cat->u.fil.filStBlk  = 0;
  cat->u.fil.filLgLen  = 0;
  cat->u.fil.filPyLen  = 0;
  cat->u.fil.filRStBlk = 0;
  cat->u.fil.filRLgLen = 0;
  cat->u.fil.filRPyLen = 0;
  cat->u.fil.filCrDat  = d_mtime(time(0));
  cat->u.fil.filMdDat  = cat->u.fil.filCrDat;
  cat->u.fil.filBkDat  = 0;

  memset(&cat->u.fil.filFndrInfo, 0, sizeof(cat->u.fil.filFndrInfo));

  cat->u.fil.filClpSize = 0;

  for (i = 0; i < 3; ++i)
    {
      cat->u.fil.filExtRec[i].xdrStABN     = 0;
      cat->u.fil.filExtRec[i].xdrNumABlks  = 0;

      cat->u.fil.filRExtRec[i].xdrStABN    = 0;
      cat->u.fil.filRExtRec[i].xdrNumABlks = 0;
    }

  cat->u.fil.filResrv = 0;

  r_makecatkey(&key, file->parid, file->name);
  r_packcatrec(&key, cat, record, &reclen);
...
int hfs_rename(hfsvol *vol, const char *srcpath, const char *dstpath)
{
  ...
  if (found)
	...
	goto done;  /* source and destination are the same */
 ...
 */
int hfs_format(const char *path, int pnum, const char *vname,
	       int nbadblocks, const unsigned long badblocks[])
{
  ...
  if (vol.vlen < 800 * (1024 >> HFS_BLOCKSZ_BITS))
    ERROR(EINVAL, "volume size must be at least 800K");

  /* initialize volume geometry */

  vol.lpa = 1 + (vol.vlen - 6) / 65536;
  ...
  /* vol.mdb.drXTExtRec[0..3] */
  /* vol.mdb.drCTExtRec[0..3] */
  ...
  ext->f.cat.u.fil.filClpSize = vol.mdb.drXTClpSiz;
  ...
  cat->f.cat.u.fil.filClpSize = vol.mdb.drCTClpSiz;
  ...
  /* flush other volume state */

  vol.flags |= HFS_VOL_UPDATE_MDB | HFS_VOL_UPDATE_ALTMDB | HFS_VOL_UPDATE_VBM;
