 ...
 *
 * $Id: medium.c,v 1.2 1997/11/08 22:51:52 rob Exp $
 */
	     ...
	     const char *name, const char *type, unsigned long len)
{
  ...
  if (strlen(name) > 32 ||
      strlen(type) > 32)
    ERROR(EINVAL, "partition name/type can be at most 32 chars");
  ...
  while (len > map.pmPartBlkCnt);

  start  = map.pmPyPartStart + len;
  remain = map.pmPartBlkCnt  - len;
